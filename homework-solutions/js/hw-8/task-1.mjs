...
let forEach = [];
numbers.forEach((element) => !(element % 3) && forEach.push(element));

let map = numbers.map((element, _, array) => element - array.length);
...
let every = numbers.every((element) => String(element).length > 1);

// console.log(forEach, map, filter, find, sort, reduce, some, every);
